...
export default function Checkin() {
  ...
  async function loadWeek() {
    ...
    const days = last7Days().reverse().map((d) => ({
       date: d,
       data: byDate[d] || null,
    ...
    <div className="min-h-screen bg-gradient-to-b from-sky-100 via-emerald-50 to-slate-100 font-sans text-slate-800 relative overflow-hidden flex flex-col">
      ...
      <div className="relative z-20 px-6 py-6 flex items-center justify-between max-w-2xl mx-auto w-full">
        ...
        >
          ...
          <span className="text-sm font-medium tracking-wide hidden md:block">Back to Sanctuary</span>
        </Link>
                    ...
                    <div key={i} className="flex flex-col items-center gap-2 relative group">
                        ...
                        <div 
                            className={`
                                ...
                                ${dayObj.isToday ? "ring-2 ring-emerald-400 ring-offset-2 ring-offset-white/50" : ""}
                            `}
                            style={{ backgroundColor: dayObj.data ? EMOTIONS[dayObj.data.emotion].color : '' }}
                        >
                            {dayObj.data ? EMOTIONS[dayObj.data.emotion].emoji : "•"}
                        </div>

                        {/* Label */}
                        <span className={`text-[9px] font-bold uppercase tracking-wider ${dayObj.isToday ? "text-emerald-600 font-extrabold" : "text-slate-400"}`}>
                            {dayObj.isToday ? "Today" : new Date(dayObj.date).toLocaleDateString('en-US', { weekday: 'narrow' })}
                        </span>
                    </div>
                ))}
            </div>
             ...
             <AnimatePresence>
                {saved && (
                    ...
                    >
                        ...
                        <h3 className="text-2xl font-semibold text-slate-800 mb-2">{t.saved || "Recorded"}</h3>
                        <p className="text-slate-500 mb-8 max-w-xs mx-auto">Your pulse has been added to your journey.</p>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-white/60 rounded-2xl p-5 border border-white/50">
                        <div className="flex items-center gap-2 mb-4 text-emerald-600">
                            <Activity size={18} />
                            <span className="font-medium text-sm">{t.energy || "Physical Energy"}</span>
                            <span className="ml-auto bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full text-xs font-bold">{energy}/5</span>
                        </div>
                        <input
                            type="range"
                            min="1"
                    ...
                    <div className="bg-white/60 rounded-2xl p-5 border border-white/50">
                        <div className="flex items-center gap-2 mb-4 text-violet-500">
                            <Brain size={18} />
                            <span className="font-medium text-sm">{t.clarity || "Mental Clarity"}</span>
                            <span className="ml-auto bg-violet-100 text-violet-700 px-2 py-0.5 rounded-full text-xs font-bold">{openness}/5</span>
                        </div>
                        <input
                            type="range"
                            min="1"
                    ...
                    >
                        {saving ? (
                            ...
                            <>
                                ...
                                <Check size={18} className={canSave ? "opacity-100" : "opacity-50"} />
                            </>
                        )}
                    </motion.button>
                </div>
